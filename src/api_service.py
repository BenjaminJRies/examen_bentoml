...
def interpret_prediction(chance: float) -> tuple[str, str]:
...
try:
    ...
    print("✅ Models loaded successfully from BentoML store")
    ...
    model_ref = joblib.load(os.path.join(models_path, 'admission_model.joblib'))
    scaler_ref = joblib.load(os.path.join(models_path, 'feature_scaler.joblib'))
    print("✅ Fallback models loaded from joblib files")

# Create BentoML service
service = bentoml.Service("admission_prediction_api")
...
def predict(input_data):
    ...
    try:
        # Extract token from input (for simplicity, we'll pass it in the request body)
        ...
        # Make prediction
        if hasattr(model_ref, 'to_runner'):
            # BentoML model
            features_scaled = scaler_ref.to_runner().run(features)
            prediction = model_ref.to_runner().run(features_scaled)[0]
        else:
            # Joblib model (fallback)
            features_scaled = scaler_ref.transform(features)
            prediction = model_ref.predict(features_scaled)[0]
...
def predict_batch(input_data):
    ...
    try:
        ...
        students_data = input_data.get("students")
