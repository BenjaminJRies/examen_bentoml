"""
Simple BentoML Service for Student Admission Prediction
Uses joblib models as backup when BentoML models are not available
"""
...
# Load models (try BentoML first, fallback to joblib)
def load_models():
    ...
    try:
        ...
        print(f"⚠️ BentoML models not available: {e}")
...
def make_prediction(student_data: Dict[str, Any]) -> float:
    ...
    if hasattr(model, 'run'):
        # BentoML runner
        features_scaled = scaler.run(features)
        prediction = model.run(features_scaled)[0]
    else:
        # Joblib model
...
service = bentoml.Service("admission_api")
